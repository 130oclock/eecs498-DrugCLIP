...
class BindingAffinityModel(BaseUnicoreModel):
    ...
    def forward(
        ...
        **kwargs
        ...
        # request all hidden states from encoder
        mol_outputs = self.mol_model.encoder(
            mol_x, padding_mask=mol_padding_mask, attn_mask=mol_graph_attn_bias, return_all_hidden_states=True
        )
        # mol_outputs: (x, attn_mask, delta_pair_repr, x_norm, delta_pair_repr_norm, hidden_states)
        hidden_states = mol_outputs[-1]  # list of tensors [L][B, seq_len, D]
        layer_idx = getattr(self.args, "atom_layer", -1)
        chosen_hidden = hidden_states[layer_idx]
        print("Mol_outputs in models/drugclip.py DEBUG chosen_hidden shape:", chosen_hidden.shape)  # expect [B, seq_len, D] for per-atom
        print("DEBUG pooled shape (CLS):", chosen_hidden[:,0,:].shape)
        mol_encoder_rep = mol_outputs[0]            # final encoder output (keep original global behavior)
        ...
        pocket_x, padding_mask=pocket_padding_mask, attn_mask=pocket_graph_attn_bias, return_all_hidden_states=True
        )
